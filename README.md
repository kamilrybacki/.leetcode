...
|112|[Path Sum](https://leetcode.com/problems/path-sum/)|[Rust](./src/easy/112.path-sum.rs)|Easy| 65.6 % | 28.8 % | At each node check if there are available nodes. If one is available, go down that path only with recursion, if both - check both and return OR of their respective recursion calls. If there are no paths left, check if `sum == target` (there is nowhere left to go). At each step increment sum by `val` |
